...
% ----------------------------------------------------------------------
\begin{titlepage}
    % Barre verticale bleue sur le côté gauche (exactement collée au bord gauche du papier)
    \begin{tikzpicture}[remember picture, overlay]
        \fill[color=techblue] (current page.north west) rectangle ++(1.4cm,-\paperheight);
    \end{tikzpicture}
    
    \centering
    \vspace*{-1cm}
    
    % Logo de l'école SUP'RH
    \begin{flushleft}
        \hspace*{1.8cm}
        \begin{minipage}{0.7\textwidth}
            \textbf{\color{darkslate}\Huge SUP'RH} \\
            {\color{gray}\scriptsize SCHOOL OF MANAGEMENT \& ARTIFICIAL INTELLIGENCE} \\
            \colorbox{red!80!black}{\color{white}\bfseries\tiny \hspace*{0.1cm} RECONNUE PAR L'ÉTAT \hspace*{0.1cm}}
        \end{minipage}
    \end{flushleft}
    
    \vspace*{2cm}
    
    % Type de document et diplôme
    {\color{darkslate}\Huge \textbf{RAPPORT DE STAGE}} \\[0.4cm]
    {\large \underline{\textbf{Troisième Année de Licence : Intelligence Artificielle et Business}}} \\[1.8cm]
    
    {\large Présenté par} \\[0.4cm]
    {\Large \textbf{\textit{« NAOUAL HOUSSNI »}}} \\[2cm]
    
    % Titre du projet (formaté exactement comme sur le modèle)
    {\color{techblue!60!gray}\Huge \textbf{« SmartCaravan : Système Intelligent\\ de Tracking et de Suivi de la Caravane\\ "Coding Pour Tous" »}} \\[2cm]
    
    {\large Sous la direction de} \\[0.4cm]
    {\large \textbf{M. Naoufal ROUKY} (SUP'RH) \& \textbf{M. Hamza AOUADI} (Tech57)} \\[2cm]
    
    % Membres du jury (avec puces et alignés comme dans le modèle)
    \begin{flushleft}
        \hspace*{1.8cm}
        \begin{minipage}{0.8\textwidth}
            \textbf{\color{darkslate}Membres du jury :} \\[0.3cm]
            \begin{tabular}{lll}
                $\bullet$ \textit{M. Mohamed Amine TALHAOUI} & \hspace{1cm} & \textit{Président} \\
                $\bullet$ \textit{M. Naoufal ROUKY} & \hspace{1cm} & \textit{Examinateur (SUP'RH)} \\
                $\bullet$ \textit{M. Hamza AOUADI} & \hspace{1cm} & \textit{Examinateur (Tech57)} \\
            \end{tabular}
        \end{minipage}
    \end{flushleft}
...
Je remercie chaleureusement **Pr. Mohamed Amine TALHAOUI**, mon encadrant au sein de SUP’RH, pour son accompagnement rigoureux, ses conseils pertinents et sa disponibilité tout au long de ce travail de recherche. Son encadrement a été déterminant dans la structuration de ma réflexion.
