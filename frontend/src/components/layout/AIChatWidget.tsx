    ...
    <div className="fixed bottom-6 right-6 z-[9999] font-sans">
      ...
      {!isOpen && (
        <button
          ...
          className="w-16 h-16 bg-gradient-to-tr from-[#1F3C6D] to-[#A4C639] rounded-full flex items-center justify-center shadow-2xl border-2 border-white/50 hover:scale-110 active:scale-95 transition-all duration-300 relative group animate-bounce p-0 overflow-hidden"
        >
          <img 
          ...
          <div className="absolute right-20 bg-[#1F3C6D] text-white text-xs font-bold px-3.5 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap shadow-xl">
      ...
      {isOpen && (
        <div className="w-96 h-[500px] bg-white rounded-[32px] shadow-2xl border border-slate-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300">
          ...
          {/* Panel Header */}
          <div className="bg-gradient-to-r from-[#1F3C6D] to-[#A4C639] p-4 text-white flex justify-between items-center shrink-0">
          ...
          {view === "chat" && activeConv && (
            <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
              {/* Title & Stats */}
              <div className="bg-white px-4 py-2 border-b border-slate-100 text-xs font-bold text-[#1F3C6D] flex justify-between items-center">
                  ...
                  >
                    <div
                      className={`max-w-[80%] p-3.5 rounded-2xl shadow-sm relative ${
                        msg.sender === "user"
                          ? "bg-[#A4C639] text-white rounded-br-none"
              ...
              <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-slate-100 flex gap-2 shrink-0">
                <input
                  ...
                  className="flex-1 px-4 py-2 bg-slate-100 rounded-full text-xs font-medium focus:outline-none focus:ring-2 focus:ring-[#A4C639] text-slate-800"
                />
                <button
                  type="submit"
                  disabled={!inputText.trim()}
                  className="w-9 h-9 bg-[#1F3C6D] hover:bg-[#A4C639] text-white rounded-full flex items-center justify-center transition-all disabled:opacity-50 font-bold"
                >
                  ➔
                </button>
              </form>
          ...
          {view === "history" && (
            <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden">
              <div className="p-4 border-b border-slate-100 bg-white flex justify-between items-center shrink-0">
                <span className="text-xs font-black text-[#1F3C6D]">Historique des discussions</span>
                <button
                  onClick={startNewConversation}
                  className="bg-[#A4C639] hover:bg-[#009685] text-white font-extrabold text-[10px] px-3.5 py-1.5 rounded-full transition flex items-center"
                >
                  Nouvelle discussion
                </button>
              </div>
                  ...
                  <button
                    ...
                    className={`w-full p-4 rounded-2xl text-left border transition flex items-center justify-between group ${
                      conv.id === activeConvId
                        ? "bg-white border-[#A4C639] shadow-md"
                        : "bg-white border-slate-100 hover:border-slate-200 shadow-sm"
                    }`}
                  >
                    <div className="space-y-1 max-w-[80%]">
                      <p className="text-xs font-black text-[#1F3C6D] truncate group-hover:text-[#A4C639] transition-colors">
                        {conv.title}
                      </p>
